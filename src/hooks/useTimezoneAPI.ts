...
import { TimezoneData, CitySearchResult } from '../types';

const WORLD_TIME_API_BASE = 'http://worldtimeapi.org/api';
...
export const useTimezoneAPI = () => {
  ...
  const fetchTimezoneData = useCallback(async (timezone: string): Promise<TimezoneData> => {
    setLoading(true);
    setError(null);

    try {
      const response = await fetch(`${WORLD_TIME_API_BASE}/timezone/${timezone}`);
      if (!response.ok) {
  ...
  const searchCities = useCallback(async (query: string): Promise<CitySearchResult[]> => {

...
import { workScheduleSchema, WorkScheduleFormData } from '../schemas/workScheduleSchema';
import type { RootState } from '../store';
import type { WorkHoursInputProps } from '../types';

export const WorkHoursForm = ({ className = '' }: Omit<WorkHoursInputProps, 'schedule' | 'onScheduleChange'>) => {
  const dispatch = useDispatch();
  const schedule = useSelector((state: RootState) => state.workSchedule.userSchedule);
  ...
  React.useEffect(() => {
    if (isValid && (watchedValues.startTime !== schedule.startTime || watchedValues.endTime !== schedule.endTime)) {
  ...
  }, [watchedValues, isValid, schedule, handleSubmit]);

  return (
    <div className={`p-6 bg-white rounded-lg shadow-sm border ${className}`}>
      <h3 className="text-lg font-semibold text-gray-800 mb-4">Your Work Schedule</h3>
